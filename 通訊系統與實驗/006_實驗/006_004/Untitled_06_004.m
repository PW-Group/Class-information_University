...
[t,s] = przz(bits,bitrate);  % 產生訊號波形 Unipolar NRZ s(t), 訊號時間長度 Tb = 1 sec 
[t2,s1]= przz(1, bitrate); % s1(t) = 1, 訊號時間長度 Tb = 1 sec, 
h = fliplr(s1);  % 匹配濾波器 h(t) = s1(Tb - t)

subplot(5,1,1);
plot(t,s,'LineWidth',3);
axis([0 t(end)+2 -1.1 1.1]);
title('Transmited bits = [1 0 1 0 1 0], 0<=t<=1 s1(t)=1 as bit = 1 and s2(t)=0 as bit = 0 ');
grid on;
...
noise = wgn(1, length(s), 10); %相加性白色高斯雜訊， 例如 10dBW = 10*log10, 0dbW = 10*log10^0, -10dbW =  10*log10^-1
...
axis([0 t(end)+2 -1.5 1.5]);
hold on
threshold_line = 0*ones(1, length(vq)); %畫門檻值之線
...
subplot(5,1,5);
[t4,s4] = przz(vb,bitrate); %為與原訊號 s 比較，重建訊號為 s4 
plot(t4,s4,'red','LineWidth',3);
title('Received bits');
axis([0 t4(end)+1 -1.1 1.1]);