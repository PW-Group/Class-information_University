...
function conv = graphConvolve(a,b)
...
for m = 1:mlen
    ...
    xlabel('Overlap of x[m] and h[n-m]');
    conv(m) = sum((a(1:m).*b((end-m+1):end)));
    subplot(212); hold off; stem(xconv(1:m),conv(1:m),'k'); xlim([-length(b) mlen]);
    xlabel('Convolution in Progress');
    xb = xb+1;
    pause(1);
 %   M(m+2) = getframe;
end